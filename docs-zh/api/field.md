...
---
`<Field />`将自动连接到Formik的输入。它使用了`name`属性与Formik状态匹配。`<Field />`将默认为HTML`<input />`元件。
...
要么是React组件，要么是要呈现的HTML元素的名称。也就是说，以下之一：
...
当你是**不**使用自定义组件，您需要访问由其创建的基础DOM节点`Field`（例如打电话`focus`），将回调传递给`innerRef`反而道具。
...
-   同步：如果无效，则返回a`string`包含错误消息或返回`undefined`。

-   异步：返回一个抛出一个的Promise`string`包含错误消息。这就像Formik一样`validate`，而不是返回`errors`对象，它只是一个`string`。
...
注意：要允许i18n库，可以使用TypeScript类型`validate`略微放松，让你返回一个`Function`（例如。`i18n('invalid')`）。
